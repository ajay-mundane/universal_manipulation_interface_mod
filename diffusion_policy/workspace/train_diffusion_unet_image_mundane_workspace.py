...
class TrainDiffusionUnetImageMundaneWorkspace(BaseWorkspace):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                def log_action_mse(step_log, category, pred_action, gt_action):
                    # Bimanual joint space control: 14D total (7D per robot × 2 robots)
                    # Each robot: 6 joint positions + 1 gripper width = 7D
                    B, T, _ = pred_action.shape
                    pred_action = pred_action.view(B, T, -1, 7)  # 2 robots × 7D each
                    gt_action = gt_action.view(B, T, -1, 7)
                    
                    # Overall MSE
                    step_log[f'{category}_action_mse_error'] = torch.nn.functional.mse_loss(pred_action, gt_action)
                    
                    # Joint positions MSE (first 6 dims per robot)
                    step_log[f'{category}_action_mse_error_joints'] = torch.nn.functional.mse_loss(pred_action[..., :6], gt_action[..., :6])
                    
                    # Gripper width MSE (last dim per robot)
...
def main(cfg):
    workspace = TrainDiffusionUnetImageMundaneWorkspace(cfg)
    workspace.run()
